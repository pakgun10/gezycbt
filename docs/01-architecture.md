...
`identity_fields_json` practice hanya boleh memilih field dari katalog server: `name` wajib; `institution`, `class`, `student_number`, dan `department` dapat optional/wajib. Batas katalog baseline masing-masing adalah 200, 200, 150, 100, dan 100 karakter; konfigurasi boleh memilih batas yang lebih kecil. Setiap definisi memuat key stabil, label, type `TEXT`, required, batas panjang, serta optional `allowedValues` yang sudah dinormalisasi. Client tidak dapat mengirim field arbitrer di luar konfigurasi schedule. Nilai divalidasi, di-trim, diperiksa required/maxLength/allowedValues, lalu diserialisasi deterministik sebagai identity snapshot. Untuk laporan, `name`, `institution`, dan `class` diproyeksikan ke kolom snapshot; field lain berada di `identity_extra_json`. Setelah session berhasil dibuat, snapshot tidak dapat diedit; koreksi identitas memerlukan session latihan baru bila attempt policy mengizinkan, tanpa mengubah histori session lama.
