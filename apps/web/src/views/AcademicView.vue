...
const rosterClass = ref<ClassRecord | null>(null);
const rosterMembers = ref<readonly ClassMemberProfile[]>([]);
const rosterSelectedIds = ref<string[]>([]);
const rosterCandidates = ref<readonly StaffUser[]>([]);
...
async function openRoster(item: ClassRecord): Promise<void> {
  rosterClass.value = item;
  rosterLoading.value = true;
  rosterError.value = "";
  try {
    const [members, candidates] = await Promise.all([
      api.classMembers(item.id),
      api.users("", "PARTICIPANT"),
    ]);
    rosterMembers.value = members;
    rosterSelectedIds.value = members.map((member) => member.participantId);
    rosterCandidates.value = candidates.items.filter(
      (candidate) => candidate.status === "ACTIVE",
    );
  } catch (cause) {
    rosterError.value = messageFrom(cause, "Roster kelas belum tersedia.");
  } finally {
...
async function searchRosterParticipants(): Promise<void> {
  rosterLoading.value = true;
  rosterError.value = "";
  try {
    const result = await api.users(rosterSearch.value, "PARTICIPANT");
    rosterCandidates.value = result.items.filter(
      (candidate) => candidate.status === "ACTIVE",
    );
  } catch (cause) {
    rosterError.value = messageFrom(cause, "Daftar peserta belum tersedia.");
  } finally {
    rosterLoading.value = false;
  }
}
function isRosterSelected(id: string): boolean {
  return rosterSelectedIds.value.includes(id);
}
function toggleRosterParticipant(id: string): void {
  rosterSelectedIds.value = isRosterSelected(id)
    ? rosterSelectedIds.value.filter((current) => current !== id)
    : [...rosterSelectedIds.value, id];
}
async function saveRoster(): Promise<void> {
  if (!rosterClass.value) return;
  const currentIds = rosterMembers.value.map((member) => member.participantId);
  const removed = currentIds.some((id) => !isRosterSelected(id));
  if (
    removed &&
    !window.confirm("Peserta yang dihapus tidak lagi mendapat ujian dari target kelas ini. Lanjutkan?")
  )
    return;
  rosterSaving.value = true;
  rosterError.value = "";
  try {
    const members = await api.updateClassMembers(
      rosterClass.value.id,
      rosterSelectedIds.value,
    );
    rosterMembers.value = members;
    rosterSelectedIds.value = members.map((member) => member.participantId);
  } catch (cause) {
    rosterError.value = messageFrom(cause, "Roster kelas belum dapat disimpan.");
  } finally {
...
<template>
  ...
  <section v-if="rosterClass" class="card roster-panel" aria-labelledby="roster-title"><div class="between"><div><p class="eyebrow">Kelola peserta</p><h2 id="roster-title">{{ rosterClass.name }} ({{ rosterClass.code }})</h2><p class="muted">Tambahkan atau hapus peserta dari kelas ini. Perubahan memengaruhi eligibility ujian yang menargetkan kelas.</p></div><button class="btn-quiet" type="button" @click="closeRoster">Tutup</button></div><div v-if="rosterError" class="alert alert-error" role="alert">{{ rosterError }}</div><p v-if="rosterLoading" class="muted">Memuat roster…</p><template v-else><div class="roster-search"><input v-model="rosterSearch" placeholder="Cari nama atau username peserta" @keydown.enter.prevent="searchRosterParticipants" /><button class="btn-secondary" type="button" @click="searchRosterParticipants">Cari peserta</button></div><div v-if="rosterCandidates.length" class="roster-candidates"><label v-for="candidate in rosterCandidates" :key="candidate.id" class="roster-option"><input type="checkbox" :checked="isRosterSelected(candidate.id)" @change="toggleRosterParticipant(candidate.id)" /><span><strong>{{ candidate.displayName }}</strong><small>{{ candidate.username }}</small></span></label></div><p v-else class="muted">Ketik nama atau username untuk mencari peserta aktif.</p><div class="roster-summary"><strong>{{ rosterSelectedIds.length }} peserta dipilih</strong><span class="muted">{{ rosterMembers.length }} anggota saat ini</span></div><div v-if="rosterMembers.length" class="roster-current"><div v-for="member in rosterMembers" :key="member.participantId" class="roster-current-row"><span><strong>{{ member.displayName }}</strong><small>{{ member.username }}</small></span><button class="btn-quiet" type="button" :aria-pressed="isRosterSelected(member.participantId)" @click="toggleRosterParticipant(member.participantId)">{{ isRosterSelected(member.participantId) ? 'Hapus dari pilihan' : 'Kembalikan' }}</button></div></div><div class="roster-actions"><button class="btn-primary" type="button" :disabled="rosterSaving" @click="saveRoster">{{ rosterSaving ? 'Menyimpan…' : 'Simpan anggota kelas' }}</button><button class="btn-secondary" type="button" :disabled="rosterSaving" @click="closeRoster">Batal</button></div></template></section>
  <section class="card table-card"><div v-if="loading" class="table-state">Memuat data akademik…</div><div v-else-if="tab === 'years' && years.length === 0" class="table-state"><strong>Belum ada tahun ajaran</strong><span class="muted">Buat tahun ajaran pertama.</span></div><div v-else-if="tab === 'classes' && classes.length === 0" class="table-state"><strong>Belum ada kelas</strong><span class="muted">Tambahkan kelas pada tahun ajaran aktif.</span></div><div v-else-if="tab === 'subjects' && subjects.length === 0" class="table-state"><strong>Belum ada mata pelajaran</strong><span class="muted">Tambahkan subject sebelum membuat bank soal.</span></div><div v-else class="table-scroll"><table v-if="tab === 'years'"><caption class="sr-only">Tahun ajaran</caption><thead><tr><th>Nama</th><th>Periode</th><th>Status</th><th></th></tr></thead><tbody><tr v-for="year in years" :key="year.id"><td><strong>{{ year.name }}</strong><small>ID {{ year.id }}</small></td><td>{{ formatShortDate(year.startsOn) }} – {{ formatShortDate(year.endsOn) }}</td><td><span class="badge" :class="year.isActive ? 'badge-success' : 'badge-muted'">{{ year.isActive ? 'Aktif' : 'Tidak aktif' }}</span></td><td><button v-if="!year.isActive" class="btn-quiet" type="button" @click="activate(year)">Jadikan aktif</button></td></tr></tbody></table><table v-else-if="tab === 'classes'"><caption class="sr-only">Kelas</caption><thead><tr><th>Kode</th><th>Nama</th><th>Tahun ajaran</th><th>Status</th><th></th></tr></thead><tbody><tr v-for="item in classes" :key="item.id"><td>{{ item.code }}</td><td>{{ item.name }}</td><td>{{ years.find((year) => year.id === item.academicYearId)?.name ?? item.academicYearId }}</td><td>{{ item.status === 'ACTIVE' ? 'Aktif' : 'Arsip' }}</td><td><button class="btn-secondary" type="button" :disabled="item.status !== 'ACTIVE'" @click="openRoster(item)">Kelola peserta</button></td></tr></tbody></table><table v-else><caption class="sr-only">Mata pelajaran</caption><thead><tr><th>Kode</th><th>Nama</th><th>Status</th></tr></thead><tbody><tr v-for="item in subjects" :key="item.id"><td>{{ item.code }}</td><td>{{ item.name }}</td><td>{{ item.status === 'ACTIVE' ? 'Aktif' : 'Arsip' }}</td></tr></tbody></table></div></section>
</template>
...
.roster-panel { display: grid; gap: 16px; padding: 20px; margin-bottom: 18px; }.roster-panel h2 { margin: 0 0 6px; }.roster-search { display: flex; gap: 8px; }.roster-search input { flex: 1; min-height: 44px; border: 1px solid var(--border-strong); border-radius: 8px; padding: 8px 12px; color: var(--text); background: var(--surface); }.roster-candidates { display: grid; grid-template-columns: repeat(auto-fill, minmax(230px, 1fr)); gap: 8px; max-height: 240px; overflow: auto; }.roster-option, .roster-current-row { display: flex; align-items: center; gap: 10px; padding: 10px; border: 1px solid var(--border); border-radius: 8px; }.roster-option span, .roster-current-row span { display: grid; gap: 2px; }.roster-option small, .roster-current-row small { color: var(--muted); }.roster-summary { display: flex; gap: 12px; align-items: center; }.roster-current { display: grid; gap: 8px; max-height: 300px; overflow: auto; }.roster-current-row { justify-content: space-between; }.roster-actions { display: flex; gap: 8px; justify-content: flex-end; }
@media (max-width: 700px) { .form-grid { grid-template-columns: 1fr; } }
@media (max-width: 600px) { .roster-search { display: grid; grid-template-columns: 1fr; }.roster-current-row { align-items: flex-start; flex-direction: column; }.roster-current-row button { width: 100%; }.roster-actions { display: grid; grid-template-columns: 1fr; }.roster-actions button { width: 100%; } }
</style>
