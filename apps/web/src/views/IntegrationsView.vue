...
import type { IntegrationClient, IntegrationClientDetail } from "../features/staff/types";
...
async function load(): Promise<void> {
  loading.value = true;
  error.value = "";
  try { clients.value = (await api.integrationClients()).items; } catch (cause) { error.value = messageFrom(cause, "Integrasi agent belum tersedia."); } finally { loading.value = false; }
}
...
<template>
  ...
  <section v-if="showCreate" class="card form-card"><h2>Integration client baru</h2><form class="form-grid" @submit.prevent="createClient"><label>Nama<input v-model="clientForm.name" maxlength="120" required /></label><label>Platform<select v-model="clientForm.platformHint"><option>HIVEKEEP</option><option>HERMES</option><option>OTHER</option></select></label><label>Owner user ID<input v-model="clientForm.ownerUserId" inputmode="numeric" pattern="[0-9]+" required /></label><label>Deskripsi<textarea v-model="clientForm.description" maxlength="500" rows="2" /></label><button class="btn-primary" type="submit" :disabled="saving">{{ saving ? "Menyimpan…" : "Buat client" }}</button></form></section>
