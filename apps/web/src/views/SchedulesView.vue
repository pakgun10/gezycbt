...
const selectedExam = computed(
  () => exams.value.find((exam) => exam.revisionId === form.value.examRevisionId) ?? null,
);
const inheritedDurationSeconds = computed(
  () => selectedExam.value?.durationSeconds ?? form.value.durationSeconds,
);
...
function formatDuration(seconds: number): string {
  if (!Number.isFinite(seconds) || seconds < 1) return "Belum dipilih";
  if (seconds % 60 === 0) return `${seconds / 60} menit`;
  const minutes = Math.floor(seconds / 60);
  const remaining = seconds % 60;
  return minutes > 0 ? `${minutes} menit ${remaining} detik` : `${remaining} detik`;
}
function applyExamDuration(): void {
  const exam = exams.value.find((item) => item.revisionId === form.value.examRevisionId);
  if (exam) form.value.durationSeconds = exam.durationSeconds;
}
...
function examLabel(exam: ExamSummary): string { return `${exam.title} · ${formatDuration(exam.durationSeconds)} · revision #${exam.revisionId}`; }
...
.inherited-duration { display: grid; align-content: start; gap: 6px; min-height: 44px; padding: 1px 0; }
.inherited-duration .field-label { font-weight: 600; }
.inherited-duration strong { min-height: 44px; display: flex; align-items: center; padding: 8px; border: 1px solid var(--border); border-radius: 8px; color: var(--text); background: var(--canvas); font-weight: 500; }
.inherited-duration small { color: var(--subtle); font-weight: 400; }
