...
Build artifacts in CI, copy them to a new directory, and run `deploy.sh <release-id> <artifact-dir>`. The script validates the release ID, copies into an immutable release directory, records checksums, runs forward-only migrations, atomically switches `/opt/gezycbt/current`, restarts the API, and verifies readiness. A failed readiness check restores the previous symlink; migration rollback is never guessed.
